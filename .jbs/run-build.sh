...
if [ -n "" ]; then
  echo "Setting version to 1.9.4 to match enforced version"
  mvn -B -e -s "/root/software/settings/settings.xml" -t "/root/software/settings/toolchains.xml" org.codehaus.mojo:versions-maven-plugin:2.8.1:set -DnewVersion="1.9.4" | tee /root/project/logs/enforce-version.log
fi
